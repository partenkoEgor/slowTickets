/*
  ...
  Листы
   ...
   - API - чужие ГЕО, PSP - чужие ГЕО — 2 листа Tracking, по одному на
     тип. Структура (заголовки) одинаковая на обоих листах — та же, что
     в исходном скрипте (Тип платежа, Дата фиксации, Дата создания
     тикета, Дата входа в статус, Processing time, ГЕО, Субагент,
     Департамент, Ticket ID, User ID, Статус зависания, Причина
     зависания, Актуальный статус, Результат, Комментарий).
     Тип платежа и Департамент заполняются так же, как в исходном
     скрипте (см. CONFIG.paymentTypeMap и CONFIG.fieldMap).
   - Архив API - чужие ГЕО, Архив PSP - чужие ГЕО — сюда переносятся
...
const CONFIG = {
  ...
  types: [
    { value: 'API', trackingSheet: 'API - чужие ГЕО', archiveSheet: 'Архив API - чужие ГЕО' },
    { value: 'PSP', trackingSheet: 'PSP - чужие ГЕО', archiveSheet: 'Архив PSP - чужие ГЕО' },
  ],
